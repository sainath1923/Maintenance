...
export default function AttendanceModal({ token, open, onClose }) {
  ...
  useEffect(() => { if (open) loadToday(); }, [open]);
  ...
  const doPunch = (type) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setError('Could not get your location.'); setBusy(false); },
      { enableHighAccuracy: true },
    );
  };

  const handleClose = () => { setError(''); setMsg(''); onClose(); };
  ...
  return (
    <Modal open={open} onCancel={handleClose} title="Attendance" footer={null} width={380}>
      ...
      <div className="attendance-actions">
        {!attendanceToday.punchIn && (
          <button className="btn-primary" disabled={busy} onClick={() => doPunch('in')}>
            {busy ? 'Please wait…' : 'Punch In'}
          </button>
        )}
        {attendanceToday.punchIn && !attendanceToday.punchOut && (
          <button className="btn-primary" disabled={busy} onClick={() => doPunch('out')}>
